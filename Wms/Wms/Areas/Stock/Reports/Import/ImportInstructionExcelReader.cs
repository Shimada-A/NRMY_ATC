namespace Wms.Areas.Stock.Reports.Import
{
    using System.Collections.Generic;
    using System.Web;
    using OfficeOpenXml;
    using Share.Reports.Import;
    using Wms.Areas.Stock.ViewModels.ImportInstruction;

    public class ImportInstructionExcelReader<T> : IReportReader<T> where T : ViewModels.ImportInstruction.ImportInstructionOutputReport
    {
        /// <summary>
        /// Excelのデータを読み取る
        /// </summary>
        /// <param name="file"></param>
        /// <returns></returns>
        public IEnumerable<T> Read(HttpPostedFileBase file)
        {
            IEnumerable<T> reportData;

            using (var excel = new ExcelPackage(file.InputStream))
            {
                var workSheet = excel.Workbook.Worksheets[1];
                var endColumn = workSheet.Dimension.End.Column;
                var endRow = LastRow(workSheet, endColumn);

                var data = new List<ViewModels.ImportInstruction.ImportInstructionOutputReport>();

                for (var row = 2; row <= endRow; row++)
                {
                    // データの追加
                    data.Add(CreateReportInstance(workSheet, row));
                }

                reportData = data as IEnumerable<T>;

                return reportData;
            }
        }

        /// <summary>
        /// WorkSheetのデータからReportのインスタンスを作成する
        /// </summary>
        /// <param name="workSheet"></param>
        /// <param name="row"></param>
        /// <returns></returns>
        private ViewModels.ImportInstruction.ImportInstructionOutputReport CreateReportInstance(ExcelWorksheet workSheet, int row)
        {
            int intStockQty;
            if(workSheet.Cells[row, 5].Text.Length == 0)
            {
                intStockQty = 0;
            }
            else
            {
                intStockQty = int.Parse(workSheet.Cells[row, 5].Text);
            }

            // 情報を読み出す
            var ImportInstructionInfo = new ViewModels.ImportInstruction.ImportInstructionOutputReport
            {
                SortClass = byte.Parse(workSheet.Cells[row, 1].Text),
                Jan = workSheet.Cells[row, 2].Text,
                SortCategoryName = workSheet.Cells[row, 3].Text,
                TransferNo = workSheet.Cells[row, 4].Text,
                StockQty = intStockQty,
                Note = workSheet.Cells[row, 6].Text
            };

            return ImportInstructionInfo;
        }

        /// <summary>
        /// 値が入っている最終行を取得
        /// </summary>
        /// <param name="workSheet"></param>
        /// <param name="endColumn"></param>
        /// <returns></returns>
        private int LastRow(ExcelWorksheet workSheet, int endColumn)
        {
            var endRow = workSheet.Dimension.End.Row;

            for (; endRow > 1; endRow--)
            {
                var exists = false;
                for (int i = 1; i <= endColumn; i++)
                {
                    if (!string.IsNullOrEmpty(workSheet.Cells[endRow, i].Text))
                    {
                        exists = true;
                        break;
                    }
                }

                if (exists) break;
            }

            return endRow;
        }
    }
}
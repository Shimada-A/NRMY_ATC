namespace Wms.Areas.Ship.Reports.Import
{
    ...
    public class UploadCaseInstructionExcelReaderJan<T> : IReportReader<T> where T : ViewModels.UploadCaseInstruction.UploadCaseInstructionReportJan
    {
        /// <summary>
        /// Excelのデータを読み取る
        /// </summary>
        ...
        public IEnumerable<T> Read(HttpPostedFileBase file)
        {
            ...
            using (var excel = new ExcelPackage(file.InputStream))
            {
                ...
                var data = new List<ViewModels.UploadCaseInstruction.UploadCaseInstructionReportJan>();
        ...
        /// <returns></returns>
        private ViewModels.UploadCaseInstruction.UploadCaseInstructionReportJan CreateReportInstance(ExcelWorksheet workSheet, int row)
        {
            // 情報を読み出す
            var UploadCaseInstructionInfo = new ViewModels.UploadCaseInstruction.UploadCaseInstructionReportJan
            {
                ShipPlanDate = workSheet.Cells[row, 1].Text,
                ShipToStoreId = workSheet.Cells[row, 2].Text,
                PrioritOrder = workSheet.Cells[row, 3].Text,
                Jan = workSheet.Cells[row, 4].Text,
                InstructQty = workSheet.Cells[row, 5].Text
            };

/**
 * 仕入先検索モーダル
 * _TransporterSearchModal.cshtmlに必要
 */
var transporterSearchModal = (function () {
    var current_scrollY;
    var id = '#transporterSearchModalDialog';
    ...
    return {
        ...
        resize: function () {
            ...
            timer = setTimeout(function () { transporterSearchModal.adjust(); }, 200);
        },
        /**
         * ウィンドウを閉じる(キャンセル)
         */
        ...
        saveTemp: function () {
            ...
            var id = $("input[type='hidden'][name$='Transporter.CenterId']").eq(index).val();

            if ($(this).prop("checked")) {
                //hiddenを作る
                $("<input>", {
                    type: "hidden",
                    name: "checkedTransporters",
                    value: id
                }).appendTo("#checked");
            } else {
        ...
        saveChanges: function () {
            $("#selectedTransporter").empty();
            $("input[type='hidden'][name='checkedTransporters']").clone().appendTo("#selectedTransporter");
        },
        /**
         * モーダルダイアログを開く
         */
        open: function () {
            ...
            //画面側の選択項目をコピー
            $("#checked").empty();
            $("#selectedTransporter input[type='hidden'][name='checkedTransporters']").clone().appendTo("#checked");
            ...
            checked.each(function () {
                ...
                var target = $("input[type='hidden'][name$='Transporter.CenterId']").filter("[value='" + $(this).val() + "']");
        ...
        init: function () {
            ...
            $('#transporterClose').on('click', this.close);
            $('#transporterSelect').on('click', this.close);
            $("#transporterSelect").on("click", this.saveChanges);
